...
public class RXTXCommDriver implements CommDriver {
	...
	private final static boolean noVersionOutput = "true".equals(System
			.getProperty("gnu.io.rxtx.NoVersionOutput"));
	...
	static {
		...
		if (!JarVersion.equals(LibVersion)) {
			System.out
					.println("WARNING:  RXTX Version mismatch\n\tJar version = "
							+ JarVersion
							+ "\n\tnative lib Version = "
							+ LibVersion);
		} else if (debug) {
			...
			if (debug)
				System.out
						.println("\nRXTXCommDriver:getValidPortPrefixes() No ports prefixes known for this System.\nPlease check the port prefixes listed for "
								+ osName
								+ " in RXTXCommDriver:registerScannedPorts()\n");
		}
		int i = 0;
	...
	private void checkSolaris(String PortName, int PortType) {
		...
		for (p[0] = 97; p[0] < 123; p[0]++) {
			if (testRead(PortName.concat(new String(p)), PortType)) {
				CommPortIdentifier.addPortName(PortName.concat(new String(p)),
						PortType, this);
			}
		}
			...
			for (i = 0; i < CandidateDeviceNames.length; i++) {
				for (p = 0; p < ValidPortPrefixes.length; p++) {
					...
					if (osName.equals("Solaris") || osName.equals("SunOS"))
						...
						CommPortIdentifier
								.addPortName(PortName, PortType, this);
					}
				}
			}
		}
	 ...
	 * 
	 * Tested only with sun jdk1.3 The file gnu.io.rxtx.properties must reside
	 * in the java extension dir
	 * 
	...
	private boolean registerSpecifiedPorts(int PortType) {
		...
		try {
			...
			if (debug) {
				System.out
						.println("The file: gnu.io.rxtx.properties doesn't exists.");
				System.out.println(e.toString());
			}// end if
		}// end catch

		if (debug)
			System.out.println("checking for system-known ports of type "
					+ PortType);
		if (debug)
			System.out.println("checking registry for ports of type "
					+ PortType);
		...
		default:
			if (debug)
				System.out.println("unknown port type " + PortType
						+ " passed to RXTXCommDriver.registerSpecifiedPorts()");
		}
	...
	private void registerScannedPorts(int PortType) {
		...
		if (debug)
			System.out.println("scanning device directory " + deviceDirectory
					+ " for ports of type " + PortType);
		if (osName.equals("Windows CE")) {
			String[] temp = { "COM1:", "COM2:", "COM3:", "COM4:", "COM5:",
					"COM6:", "COM7:", "COM8:" };
		...
		if (CandidateDeviceNames == null) {
			if (debug)
				System.out
						.println("RXTXCommDriver:registerScannedPorts() no Device files to check ");
			return;
		}
		...
		case CommPortIdentifier.PORT_SERIAL:
			if (debug)
				System.out
						.println("scanning for serial ports for os " + osName);

			/*
			 * There are _many_ possible ports that can be used on Linux. See
			 * below in the fake Linux-all-ports case for a list. You may add
			 * additional ports here but be warned that too many will
			 * significantly slow down port enumeration. Linux 2.6 has udev
			 * support which should be faster as only ports the kernel finds
			 * should be exposed in /dev
			 * 
			 * See also how to override port enumeration and specifying port in
			 * INSTALL.
			 * 
			 * taj
			 */
			...
			if (osName.equals("Linux")) {
				String[] Temp = { "ttyS", // linux Serial Ports
						"ttySA", // for the IPAQs
			...
			} else if (osName.equals("NetBSD")) // FIXME this is probably wrong
			{
				...
				String[] Temp = {
						...
						"tty2A", "tty3A", "tty4A", "tty5A", "tty6A", "tty7A",
						"tty8A", "tty9A", "tty10A", "tty11A", "tty12A",
						"tty13A", "tty14A", "tty15A",
						"tty16A",
						"ttyu1A", // OSR5 USB-serial ports
						"ttyu2A", "ttyu3A", "ttyu4A", "ttyu5A", "ttyu6A",
						"ttyu7A", "ttyu8A", "ttyu9A", "ttyu10A", "ttyu11A",
						"ttyu12A", "ttyu13A", "ttyu14A", "ttyu15A", "ttyu16A" };
				CandidatePortPrefixes = Temp;
				...
				if (debug)
					System.out
							.println("No valid prefixes for serial ports have been entered for "
									+ osName
		...
		case CommPortIdentifier.PORT_PARALLEL:
			if (debug)
				System.out.println("scanning for parallel ports for os "
						+ osName);
			/**
			 * Get the Parallel port prefixes for the running os Holger Lehmann
			 * July 12, 1999 IBM
			 */
			if (osName.equals("Linux")
			/*
			 * || osName.equals("NetBSD") FIXME || osName.equals("HP-UX") FIXME
		...
		default:
			if (debug)
				System.out.println("Unknown PortType " + PortType
						+ " passed to RXTXCommDriver.registerScannedPorts()");
		}
		registerValidPorts(CandidateDeviceNames, CandidatePortPrefixes,
				PortType);
	}
	 ...
	 *         its openPort() method. PortName is a string that was registered
	 *         earlier using the CommPortIdentifier.addPortName() method.
	 *         getCommPort() returns an object that extends either SerialPort or
	 *         ParallelPort.
	 */
	public CommPort getCommPort(String PortName, int PortType) {
		if (debug)
			System.out.println("RXTXCommDriver:getCommPort(" + PortName + ","
					+ PortType + ")");
		try {
			switch (PortType) {
			case CommPortIdentifier.PORT_SERIAL:
