...
final public class RXTXPort extends SerialPort {
	...
	protected final static boolean debug_read = true;
	protected final static boolean debug_read_results = true;
	protected final static boolean debug_write = true;
	...
	public synchronized void setSerialPortParams(int b, int d, int s, int p)
			throws UnsupportedCommOperationException {
		if (debug)
			z.reportln("RXTXPort:setSerialPortParams(" + b + " " + d + " " + s
					+ " " + p + ") called");
	 ...
	 */
	private native boolean nativeSetSerialPortParams(int speed, int dataBits,
	...
	public int getDataBits() {
		if (debug)
			z.reportln("RXTXPort:getDataBits() called and returning "
					+ dataBits);
		return dataBits;
	}
	...
	public int getStopBits() {
		if (debug)
			z.reportln("RXTXPort:getStopBits() called and returning "
					+ stopBits);
		return stopBits;
	}
	...
	public void setFlowControlMode(int flowcontrol) {
		if (debug)
			z.reportln("RXTXPort:setFlowControlMode( " + flowcontrol
					+ " ) called");
	...
	*/
	...
	public boolean isReceiveTimeoutEnabled() {
		if (debug)
			z.reportln("RXTXPort:isReceiveTimeoutEnabled() called and returning "
	...
	public int getReceiveTimeout() {
		if (debug)
			z.reportln("RXTXPort:getReceiveTimeout() called and returning "
	...
	public void enableReceiveThreshold(int thresh) {
		if (debug)
			z.reportln("RXTXPort:enableReceiveThreshold( " + thresh
					+ " ) called");
		if (thresh >= 0) {
			threshold = thresh;
			NativeEnableReceiveTimeoutThreshold(timeout, threshold, InputBuffer);
	...
	*/
	...
	public int getReceiveThreshold() {
		if (debug)
			z.reportln("RXTXPort:getReceiveThreshold() called and returning "
					+ threshold);
		return threshold;
	}
	...
	public boolean isReceiveThresholdEnabled() {
		if (debug)
			z.reportln("RXTXPort:isReceiveThresholdEnable() called and returning"
					+ (threshold > 0));
		return (threshold > 0);
	}
	...
	*/
	public int getInputBufferSize() {
		if (debug)
			z.reportln("RXTXPort:getInputBufferSize() called and returning "
					+ InputBuffer);
		return (InputBuffer);
	}
	...
	public int getOutputBufferSize() {
		if (debug)
			z.reportln("RXTXPort:getOutputBufferSize() called and returning "
					+ OutputBuffer);
		return (OutputBuffer);
	}
	...
	protected native int readTerminatedArray(byte b[], int off, int len,
	...
	*/
	...
	public void close() {
		...
		if (debug)
			z.reportln("RXTXPort:close( " + this.name
					+ " ) setting monThreadisInterrupted");
		if (!monThreadisInterrupted) {
			removeEventListener();
		}
		if (debug)
	...
	class SerialOutputStream extends OutputStream {
		...
		public void write(byte b[]) throws IOException {
			if (debug_write) {
				z.reportln("Entering RXTXPort:SerialOutputStream:write("
			...
			try {
				...
				if (debug_write)
					z.reportln("Leaving RXTXPort:SerialOutputStream:write("
							+ b.length + ")");
			} finally {
				synchronized (IOLockedMutex) {
					IOLocked--;
	...
	class SerialInputStream extends InputStream {
		 ...
		 *      available timeout > 0, threshold = 0, blocks until timeout
		 *      occurs, returns -1 on timeout >0 >0 blocks until timeout,
		 *      returns - 1 on timeout, magnitude of threshold doesn't play a
		 *      role. 0 >0 Blocks until 1 byte, magnitude of threshold doesn't
		 *      play a role
		 */
		 ...
		 *             --------------------------------
		 *             ---------------------------------------- 0 0 blocks until
		 *             1 byte is available >0 0 blocks until timeout occurs,
		 *             returns 0 on timeout >0 >0 blocks until timeout or reads
		 *             threshold bytes, returns 0 on timeout 0 >0 blocks until
		 *             reads threshold bytes
		 */
		public synchronized int read(byte b[]) throws IOException {
			int result;
			if (debug_read)
				z.reportln("RXTXPort:SerialInputStream:read(" + b.length
						+ ") called");
			if (monThreadisInterrupted == true) {
				return (0);
			}
			...
			try {
				...
				if (debug_read_results)
					z.reportln("RXTXPort:SerialInputStream:read() returned "
							+ result + " bytes");
				return (result);
			} finally {
				synchronized (IOLockedMutex) {
		 ...
		 *             returns 0 on timeout >0 >0 blocks until timeout or reads
		 *             threshold bytes, returns 0 on timeout 0 >0 blocks until
		 *             either threshold # of bytes or len bytes, whichever was
		 *             lower.
		 */
	...
	*/
	...
	private native static void nativeStaticSetSerialPortParams(String f, int b,
			int d, int s, int p) throws UnsupportedCommOperationException;
	...
	public static void staticSetSerialPortParams(String f, int b, int d, int s,
			int p) throws UnsupportedCommOperationException {
		if (debug)
			z.reportln("RXTXPort:staticSetSerialPortParams( " + f + " " + b
					+ " " + d + " " + s + " " + p);
