 ...
 * @description 卡板入库 业务逻辑类
 */
public class FinishWareHouseCardModel extends CommonModel {
	...
	/**
	 * @param task
	 * @description 获取卡板入仓所需的参数信息 MOId MOName SOName
	 */
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"select m.MOId,m.MOName,s.SOName from ProductComplete p inner join MO m on p.MOId = m.MOId inner join SO s on m.SOId = s.SOId where p.ProductCompleteID = '"
									+ completeId + "';");
					...
					if (null != resMapsLis && 0 != resMapsLis.size()) {
						...
						for (int i = 0; i < resMapsLis.size(); i++) {
							...
							if (tempMap.containsKey(MOID)
									&& tempMap.containsKey(MONAME)
									&& tempMap.containsKey(SONAME)
									&& !StringUtils.isEmpty(tempMap.get(MOID))
									&& !StringUtils
											.isEmpty(tempMap.get(MONAME))
									&& !StringUtils
											.isEmpty(tempMap.get(SONAME))) {
	 ...
	 * @description 卡板成品入仓
	 */
	public void finishWareHouseCard(Task task) {
		TaskOperator taskOperator = new TaskOperator() {
			@Override
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"declare @retMsg nvarchar(max) declare @excep nvarchar(100) declare @retRes int exec @retRes = PDA_ProdEnterLocation '',@retMsg output,@excep,'','','"
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"select ProductCompleteID,CompleteDocNo from ProductComplete where CompleteDocNo like '%"
									+ checkStr
									+ "%' order by CompleteDocNo asc ;");
					SoapSerializationEnvelope envolop = MesWebService
							.getWSReqRes(soap);
					if (null == envolop || null == envolop.bodyIn)
						return task;
					...
					if (null != resMapsLis && 0 != resMapsLis.size()) {
						for (int i = 0; i < resMapsLis.size(); i++) {
							HashMap<String, String> tempMap = resMapsLis.get(i);
							if (tempMap.containsKey(PRODUCTCOMPLETEID)
									&& tempMap.containsKey(COMPLETEDOCNO)
									&& !StringUtils.isEmpty(tempMap
											.get(PRODUCTCOMPLETEID))
									& !StringUtils.isEmpty(tempMap
											.get(COMPLETEDOCNO))) {
								HashMap<String, String> mapItem = new HashMap<String, String>();
								mapItem.put(PRODUCTCOMPLETEID,
										tempMap.get(PRODUCTCOMPLETEID));
								mapItem.put(COMPLETEDOCNO,
										tempMap.get(COMPLETEDOCNO));
								lisMaps.add(mapItem);
							}
						}
