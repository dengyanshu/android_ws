...
public class MaterialInventoryModel extends CommonModel {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty("SQLString",
							"select QTY,StockLocation from stockLOC where LotSN = '"
									+ lotSN + "'");
	...
	/**
	 * @param task
	 * @description 物料盘点 如果传入的 @Labar(查看存储过程 Txn_MaterialsInventory --物料盘点) = CK
	 *              物料检查 SM = 数据保存
	 */
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"declare @ReturnMessage nvarchar(max)  = ' ' declare @res int declare @excep nvarchar(100) exec @res = Txn_MaterialsInventory '',@ReturnMessage output,@excep output,'1'"
									+ ",'','','','','','','','','','"
									+ pas[0]
									+ "','"
									+ pas[1]
