...
public class TiebiaozhuandsnModel extends CommonModel {

	public void tiebiaozhuandsn(Task task) {
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					String pcbasn = Params[6];
					String custmodel = Params[7];
					String mac = Params[8];
					String dsn = Params[9];
					String custsn = Params[10];

					Log.i(TAG, "任务数据是:" + Params[0] + "," + Params[1] + ","
							+ Params[2] + "," + Params[3] + "," + Params[4]
							+ "," + Params[5]+","+Params[6]+","+Params[7]+","+Params[8]+","+Params[9]+","+Params[10]);

					String sql = " declare @res int declare @message nvarchar(max) declare @exception nvarchar(100)  exec @res= [Txn_DIP-PasteLabelConverDSN_NOLMS]    "
							+" @I_OrBitUserId='"+useid+"',@I_OrBitUserName='"+usename+"',@I_ResourceId='"+resourceid+"',@I_ResourceName='"+resourcename+"',"
							+ "@MOId='"+moid+"',@MOName='"+moname+"',@PCBASN='"+pcbasn+"',@CustModel='"+custmodel+"',@MAC='"+mac+"',@DSN='"+dsn+"',@CustSN='"+custsn+"',"
