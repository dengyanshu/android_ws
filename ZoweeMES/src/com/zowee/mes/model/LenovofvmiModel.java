...
public class LenovofvmiModel extends CommonModel {

	public void lenovofvmi(Task task) {
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					String sql = " declare @res int declare @message nvarchar(max) declare @exception nvarchar(100)  exec @res= [Txn_DataChainSMTWatchSecond]   "
							+" @I_OrBitUserId='"+useid+"',@I_OrBitUserName='"+usename+"',@I_ResourceId='"+resourceid+"',@I_ResourceName='"+resourcename+"',"
							+ "@MOId='"+moid+"',@LotSN='"+sn+"',@DefectcodeSn='"+errorcode+"',	"
