...
public class SMTSCNewModel extends CommonModel {
	...
	public void SMTSC_New2(Task task) {
				...
				synchronized (this) {
					try {
						...
						String Sql = "declare @ReturnMessage nvarchar(max) = ' '  declare @Return_value int declare @ExceptionFieldName nvarchar(100) declare @Return_FinishQty int='0' exec @Return_value = Txn_SMTIssueExeNew_JM4d @I_ExceptionFieldName = @ExceptionFieldName out,@I_ReturnMessage=@ReturnMessage out,@Returnint =@Return_FinishQty out,@I_OrBitUserName='"
								...
								+ scanSN
								+ "',@paol ="
								+ OnepanelQTy+",@rail='"+railName+"',@xiaob=1  select @ReturnMessage as I_ReturnMessage,@Return_value as I_ReturnValue,@Return_FinishQty as FinishQty";//
						soap.addWsProperty("SQLString", Sql);
						SoapSerializationEnvelope envolop = MesWebService
								.getWSReqRes(soap);
						if (null == envolop || null == envolop.bodyIn)
