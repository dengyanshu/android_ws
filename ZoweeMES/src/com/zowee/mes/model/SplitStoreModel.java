		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					String splitAmount = StringUtils.conDeciStr(String
							.valueOf((((Double) taskDatas[1]).doubleValue())),
							1);
					Log.i(TAG, "splitAmount: " + splitAmount);
					soap.addWsProperty(
							"SQLString",
							" declare @retVal int declare @outDetail nvarchar(max) declare @excep nvarchar(100) declare @splitQty float select @splitQty = Qty  from lot where LotSN = '"
									+ splitLot
									+ "'; if @splitQty < "
									+ splitAmount
									+ " begin select @retVal = -1, @outDetail=' 拆分数量大于拆分批号现有的数量 ! 现有批号数量为：'+str(@splitQty) select str(@retVal),@outDetail return; end exec @retVal = TXN_LotQTYTransfer '',@outDetail output,@excep output, "
									+ " '','','','"
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty("SQLString",
							"select Qty  from lot where LotSN = '" + lotSN
									+ "';");
					SoapSerializationEnvelope envolop = MesWebService
							.getWSReqRes(soap);
					if (null == envolop || null == envolop.bodyIn)
