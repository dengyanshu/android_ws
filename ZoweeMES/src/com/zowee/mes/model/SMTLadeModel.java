		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					LotSN = Params[5];
					DefectcodeSn = Params[6];
					Sql = "declare @ReturnMessage nvarchar(max) = ' '  declare @res int declare @ExceptionFieldName nvarchar(100) exec @res = [Txn_DataChainLoadCar_E5_4D] @I_ReturnMessage=@ReturnMessage out,@I_ExceptionFieldName=@ExceptionFieldName out,@I_OrBitUserId='"
							+ userID
							+ "',@I_OrBitUserName='"
							+ UserName
							+ "',@LotS='"
							+ LotSN
							+ "',@CarInfo='"+carsn+"',@MOName='"+MOName+"',@MOID='"
							+ MoID
							+ "',@DefectcodeSn='"
							+ DefectcodeSn
