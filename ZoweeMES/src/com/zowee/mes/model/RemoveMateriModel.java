		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"declare @MOName nvarchar(50) = ' '  declare @returnMsg nvarchar(max) declare @retRes int select top 1 @MOName= isnull(m.MOName,'') from LotOnSMT l inner join MO m on l.MOId = m.MOId where "
									+ " l.SMTLineNO = '"
									+ docNo
									+ "' and l.SLotNO = '"
									+ sLotNo
									+ "'  order by l.LotOnSMTId desc;"
									+ " exec @retRes = Txn_SMTItemUnload @returnMsg output,'"
									+ MyApplication.getMseUser().getUserId()
									+ "','',@MOName,'"
									+ staLotSN
									+ "' select @retRes,@returnMsg");
