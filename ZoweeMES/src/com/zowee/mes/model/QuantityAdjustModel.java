					...
					if (null == resDataSet)
						...
						if (null != resMapsLis && 0 != resMapsLis.size()) {
							...
							for (int i = 0; i < resMapsLis.size(); i++) {
								HashMap<String, String> tempMap = resMapsLis
										.get(i);
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					soap.addWsProperty(
							"SQLString",
							"declare @ReturnMessage nvarchar(max) = ' '  declare @res int declare @excep nvarchar(100) exec @res = Txn_WMSLotQtyChange '',@ReturnMessage output,@excep output,'1'"
									+ ",'','','"
