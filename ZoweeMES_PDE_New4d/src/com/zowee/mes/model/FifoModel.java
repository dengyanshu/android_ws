...
public class FifoModel extends CommonModel
{
		...
		{		
			...
			//	soap.addWsProperty("SQLString", "if SUBSTRING('"+lotSn+"',1,1) <>'M'  begin select '条码有误,请检查!' as BoxSN,'' LotSN,'' 库位   end else begin "+
			  //  "SELECT  TOP (100) PERCENT dbo.POItemLot.BoxSN AS BoxSN, dbo.StockLoc.LotSN, dbo.stockLOC.StockLocation AS 库位  FROM  dbo.Lot inner join dbo.stockLOC on dbo.lot.LotSN=dbo.StockLoc.LotSN inner join dbo.POItemLot  on  dbo.stockloc.LotSN=dbo.POItemLot.LotSN  WHERE dbo.Lot.LotSN < '"+lotSn+"' ORDER BY dbo.StockLoc.Createdate end ");
				soap.addWsProperty("SQLString", "exec Txn_WMSRMFIFOQuery '"+lotSn+"';");
				try 
				{
	...
	public List<HashMap<String,String>> getFifoScanRes(Task task)
	{
		...
		SoapSerializationEnvelope envolop = (SoapSerializationEnvelope)task.getTaskData();
	...
	//	Log.i(TAG, "aa");
		...
		//List<HashMap<String,String>> resArr = new ArrayList<HashMap<String,String>>();
		for(int i=0;i<resDataSet.size();i++)
		{
			String regex = "=|;";
			List<int[]> indes = StringUtils.getMatStrIndex(regex, resDataSet.get(i).toString());
			...
			if(null !=indes)
			{
				...
				{
					if(0==j)
						key = resDataSet.get(i).substring(0, indes.get(j)[0]).trim();
					else
						key = resDataSet.get(i).substring(indes.get(j-1)[1],indes.get(j)[0]).trim();
					value = resDataSet.get(i).substring(indes.get(j)[1], indes.get(j+1)[0]).trim();
