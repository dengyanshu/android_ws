		...
		{		
			...
			public Task doTask(Task task)
			{
				try {
					...
					soap.addWsProperty("SQLString", "select distinct s.SOId,sr.SORootName from  SORoot sr inner join SO s on sr.SORootID = s.SORootID  where sr.SORootID = '"+soRootId+"';");
				...
				//	Log.i(TAG, ""+resMapsLis);
					...
					if(null!=resMapsLis&&0!=resMapsLis.size())
					{
						for(int i=0;i<resMapsLis.size();i++)
						{
							...
							if(tempMap.containsKey(SOROOTNAME)&&!StringUtils.isEmpty(tempMap.get(SOROOTNAME))&&tempMap.containsKey(SOID)&&!StringUtils.isEmpty(tempMap.get(SOID)))
							{
								//Log.i(TAG,"COME IN");
								HashMap<String,String> mapItem = new HashMap<String,String>();
							//	mapItem.put(SONAME, tempMap.get(SONAME));
								Log.i(TAG, ""+tempMap.get(SOID)+" "+tempMap.get(SOROOTNAME));
		...
		{		
			...
			public Task doTask(Task task)
			{
				try {
					...
					//Log.i(TAG, ""+dateTime);
					soap.addWsProperty("SQLString", "declare @retMsg nvarchar(max) declare @excep nvarchar(100) declare @retRes int exec @retRes = PDA_FinishedScanWarehouse '',@retMsg output,@excep output," +
				"'','','"+MyApplication.getMseUser().getUserId()+"','"+MyApplication.getMseUser().getUserName()+"','','','','','','','"+cardSN+"','"+dateTime+"','"+dnName+"','"+soId+"','"+soRootId+"','"+soRootName+"'  select @retRes,@retMsg");
					SoapSerializationEnvelope envolop = MesWebService.getWSReqRes(soap);
					if(null==envolop||null==envolop.bodyIn)
						return task;
		...
		{		
			...
			public Task doTask(Task task)
			{
				try {
					...
					Soap soap = MesWebService.getMesEmptySoap();
					soap.addWsProperty("SQLString", "select d.DNName,d.SORootID,c.CustomerName,c.CustomerDescription  from  DN d inner join  Customer c on d.CustomerID = c.CustomerId where d.DNName like '%"+checkStr+"%' order by d.DNName asc ; ");
					SoapSerializationEnvelope envolop = MesWebService.getWSReqRes(soap);
					if(null==envolop||null==envolop.bodyIn)
						return task;
					List<String> resDataSet = MesWebService.WsDataParser.getResDatSet(envolop.bodyIn.toString());
					...
					if(null!=resMapsLis&&0!=resMapsLis.size())
					{
						...
						{
							HashMap<String,String> tempMap = resMapsLis.get(i);
							if(tempMap.containsKey(DNNAME)&&!StringUtils.isEmpty(tempMap.get(DNNAME))&&tempMap.containsKey(SOROOTID)&&!StringUtils.isEmpty(tempMap.get(SOROOTID)))
							{
								HashMap<String,String> mapItem = new HashMap<String,String>();
								mapItem.put(DNNAME, tempMap.get(DNNAME));
								mapItem.put(SOROOTID, tempMap.get(SOROOTID));
								mapItem.put(CUSTOMERNAME, tempMap.get(CUSTOMERNAME));
								mapItem.put(CUSTOMERDESCRIPTION, tempMap.get(CUSTOMERDESCRIPTION));
								lisMaps.add(mapItem);
							}
						}
