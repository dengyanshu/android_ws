...
public class PalletOutModel extends CommonModel 
{
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					String flag = Params[7];
					String flag2 = Params[8];
					
					String sql = " declare @res int declare @message nvarchar(max)  exec @res= [Txn_PalletsnShipmentNOBond]  "
							+" @I_OrBitUserName='"+usename+"',@I_OrBitUserId='"+useid+"',@I_ResourceId='"+resourceid+"',@I_ResourceName='"+resourcename+"', "
							+" @PalletLotSN ='"+lotsn+"',@SNtype='"+type+"',@MONAME='"+moname+"', @flag="+flag+",@IsOutByProduct="+flag2+","
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
				    ...
				    String  Params = (String) task.getTaskData();
					String sql = " SELECT  MOName FROM dbo.CartoonBox INNER JOIN mo ON MO.MOId = CartoonBox.MOId WHERE CartoonBoxSN='"+Params+"' "+
               " UNION ALL SELECT    MOName FROM dbo.PalletSN   INNER JOIN mo ON MO.MOId = PalletSN.MOId WHERE PalletLotSN='"+Params+"'";
					soap.addWsProperty("SQLString", sql);
					SoapSerializationEnvelope envolop = MesWebService
							.getWSReqRes(soap);
					if (null == envolop || null == envolop.bodyIn)
