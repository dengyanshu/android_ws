		...
		{		
			...
			public Task doTask(Task task)
			{
				try {
					...
					soap.addWsProperty("SQLString", "select sr.SORootName from  SORoot sr where sr.SORootID = '"+soRootId+"';");
					...
					if(null!=resMapsLis&&0!=resMapsLis.size())
					{
						for(int i=0;i<resMapsLis.size();i++)
						{
							...
							if(tempMap.containsKey(SOROOTNAME)&&!StringUtils.isEmpty(tempMap.get(SOROOTNAME)))
							{
								//Log.i(TAG,"COME IN");
		...
		{			
			...
			public Task doTask(Task task)
			{
				try 
				{
					...
					Soap soap = MesWebService.getMesEmptySoap();
					soap.addWsProperty("SQLString", "declare @retMsg nvarchar(max) declare @excep nvarchar(100) declare @retRes int exec @retRes = Txn_FinishedProductDelivery '',@retMsg output,@excep output," +
				"'','','"+MyApplication.getMseUser().getUserId()+"','"+MyApplication.getMseUser().getUserName()+"','','','','','','','"+datetime+"','"+dnName+"','"+cardSN+"','"+pierNum+"','"+soRootId+"','"+soRootName+"'  select @retRes,@retMsg");
					SoapSerializationEnvelope envolop = MesWebService.getWSReqRes(soap);
					if(null==envolop||null==envolop.bodyIn)
						return task;
					List<String> resDataSet = MesWebService.WsDataParser.getResDatSet(envolop.bodyIn.toString());
