...
public class MESUpdateModel extends CommonModel 
{
	...
	public MESUpdate getLastestMesUpdate() throws Exception
	{
		...
		if(200==conn.getResponseCode())
			...
			return null;
		MESUpdate lastestEdition = MesUpdateParse.getParseMesUpdateObj(inStream);
	...
	{
		if(null==task||null==task.getActivity())return;
		...
		{		
			...
			public Task doTask(Task task) 
			{
				...
				catch (Exception e) 
				{
					...
					task.setTaskResult(task.getActivity().getString(R.string.mesupdate_fail));
	...
	{
		if(null==task||null==task.getActivity()||null==task.getTaskResult()) return;
		BackgroundService.addTask(task);
