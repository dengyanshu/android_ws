...
public class SoundEffectPlayService
{
	...
	{
		public static int OK_MUSIC = soundPool.load(MyApplication.getApp(), R.raw.ok,1);
		public static int ERROR_MUSIC = soundPool.load(MyApplication.getApp(), R.raw.error, 1);
	...
	/**
	 * @param resId 
	 * @param leftVolume 左音道音量
	 * @param rightVolume 右音道音量 
	 * @param priority 优先级 ，0最低
	 * @param isLoop 是否循环 0是不循环，-1是永远循环
	 * @param rate 播放的速率  0.5-2.0之间。1为正常速度
	 * @description 
	 */
	public static void playStream(int resId,float leftVolume,float rightVolume,int priority,int isLoop,int rate)
	{
		if(null==soundPool)
			soundPool = new SoundPool(MAXSTREAMS, STREAMTYPE, STREAMQUANTITY);
	...
	public static void playLaserSoundEffect(int resId)
	{
		...
		playStream(resId, 0.1f, 0.1f, 5, 0,1);//声音设置为
	}
