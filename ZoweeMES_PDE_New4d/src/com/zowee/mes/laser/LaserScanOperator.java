...
public class LaserScanOperator implements SerialPortEventListener
{
	...
	private static final String ERROR_BARCODESIGN ="04BFBD0000EFBFBD2C";//未扫描到条形码 激光头返回的信息
	public static final byte[] ERROR_BARCODEBYTE = HexStrConver.HexString2Bytes(ERROR_BARCODESIGN);//未扫描到条形码 激光头返回的信息的字节数组
	...
	static
	{
		...
		//serialParams = LaserConfigParser.getFinalSerialParams();//获得从配置文件中解析过来的激光串口参数
	...
	{
		if(!getLaserEnable()) return;//如果激光扫描的功能被停用的话  那么该语句将会被执行 
		createLaser();
	}	
	...
	public void createLaser()
	{
		...
		catch (Exception e)
		{
			...
			Log.i(TAG, serialParams+""+serialParams.name+" "+serialParams.baudRate+" "+serialParams.owner+" "+serialParams.dataBits+" "+serialParams.endBits+" "+serialParams.flowControl+" "+serialParams.parity);
			laserInitFailNotify(); 
		}
		...
		TaskOperator taskOperator = new TaskOperator()
		{
			...
			public Task doTask(Task task) 
			{
				...
				if(!StringUtils.isEmpty(val))
				{
					try
					{
						boolean isContainErrorCode = HexStrConver.isContainByte(ERROR_BARCODEBYTE, val.getBytes("UTF-8"));
	...
	public void startLaserScan(Activity activity)
	{
		if(null!=serialPortOper&&!serialPortOper.getSerialState())
		{
			...
			try {
				serialPortOper.getSerialPort().getOutputStream().write(85);
			} catch (IOException e) {
	 ...
	 private void setScanGpioState(boolean paramBoolean)
	  {
	    ...
	    try
	    {
	      FileWriter localFileWriter = new  FileWriter("/dev/scan");
	      if (paramBoolean);
	      for (int i = 48; ; i = 49)
	      {
...
//	}
	...
	{
		//	if(null==laserLis) throw new NullPointerException(MyApplication.getApp().getString(R.string.laserLis_notNull));
	...
	{
		Task notifyTask = new Task(activity,TaskType.LASER_INIT_FAIL,MyApplication.getApp().getString(R.string.laser_serial_initfail));
	...
	{
		if(null==laserScanOperator||null==laserScanOperator.serialPortOper||!laserScanOperator.laserLisState||laserScanOperator.serialPortOper.getSerialState())
			return false;
