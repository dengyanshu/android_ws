...
public class LaserConfigParser 
{
	...
	private static final String LASERCONFIG_LOCAL = "serialPortConfig.properties";//原始激光配置 文件路径
	private static final String LASERFUNCTION_NAME = "laserfunction_config";//是否启用激光扫描 用户自定义配置文件
	...
	//	InputStream inStream = MesSoapParser.class.getClassLoader().getResourceAsStream(LASERCONFIG_LOCAL);
		InputStream inStream = null;
		Properties pro = new Properties();
		try 
		{
			inStream = MyApplication.getApp().getAssets().open(LASERCONFIG_LOCAL);
			...
			if(null==name) name ="";
			if(null==owner) owner ="";
			if(null==tempTimeOut) tempTimeOut ="-1";
			if(null==tempBauRate) tempBauRate ="-1";
			if(null==tempDataBits) tempDataBits ="-1";
			if(null==tempEndBit) tempEndBit ="-1";
			if(null==tempPriorityBit) tempPriorityBit ="-1";
			if(null==tempFlowControl) tempFlowControl ="-1";
			...
			serialParams = new SerialParams(name, owner, timeout, bauRate, dataBits, endBit, privatyBit, flowControl);
		//	inStream.close();
		}
		catch (Exception e) 
		{
	...
	public static void mergeLaserConfig(SerialParams serialParams)
	{
		...
		SharedPreferences preferences = MyApplication.getApp().getSharedPreferences(LASERCONFIG_NAME, Context.MODE_PRIVATE);
...
//	{
//		SerialParams serialParams = null;
//		InputStream inStream = MesSoapParser.class.getClassLoader().getResourceAsStream("com/yksj/mse/laser/serialPortConfig.properties");
//		Properties pro = new Properties();
//		try 
//		{
//			pro.load(inStream);
...
//	}
    ...
    public static SerialParams getFinalSerialParams()
    {
		...
		SerialParams serialParams = new SerialParams(name, owner, timeout, bauRate, dataBits, endBit, privatyBit, flowControl);
    ...
    {
    	SharedPreferences sharedPreferences = MyApplication.getApp().getSharedPreferences(LASERFUNCTION_NAME, Context.MODE_PRIVATE);
    	if(!sharedPreferences.contains(LASERENABLE))//表明了该用户是第一次使用该应用
    	{
    		Editor editor = sharedPreferences.edit();
    		editor.putBoolean(LASERENABLE, true);
    		editor.commit();
    	}
