				...
				builder2.setPositiveButton("确定"
						...
						, new OnClickListener() {
							...
							public void onClick(DialogInterface dialog,int which) {
								...
								if(!getMOInfoOk){
									EditScanInfo.append("请先获取订单信息！\n");
									return;
								}else if(params[4].equals("")){								
									EditScanInfo.append("请先输入车量信息！\n");
									return;
								}else if(params[4].length() < 8){
									EditScanInfo.append("车量信息长度不正确！\n");
									return;
								}
		...
		case TaskType.SMT_LadeStorage:
			if (null != task.getTaskResult()) {
				...
				if (getdata.get("Error") == null) {
					...
					{
						this.EditScanInfo.append(params[4] + "入库成功\n");
					}	
					else{
						this.EditScanInfo.append(params[4] + "入库失败！\n");					 
					}
				} else {
