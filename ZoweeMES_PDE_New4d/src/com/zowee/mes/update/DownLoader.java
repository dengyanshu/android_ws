...
public class DownLoader 
{
	...
	public DownLoader(String downPath, String storePath, MESUpdateActivity activity)
	{
		super();
	...
	private  void  downFile()
	{
		...
		try
		{
			if(StringUtils.isEmpty(downPath)||StringUtils.isEmpty(storePath)||isStop||null==activity)
			{
				Task notifyErrror = new Task(activity,TaskType.DOWNCONFIG_ERROR,null);
			...
			if(200==conn.getResponseCode())
			{
				...
				Task downFileLen = new Task(activity,TaskType.DOWNFILE_LENGTH,fileLength);
				BackgroundService.addTask(downFileLen);//通知调用者下载文件的长度
				inStream = conn.getInputStream();
				byte[] bytesItem = new byte[10*1024];//缓存的大小为10K
				int len = -1;
				File file = new File(storePath);
				if(file.exists()) file.delete();//如果文件存在的话 删除该文件
				//outStream = new FileOutputStream(file);
				outStream = activity.openFileOutput(storePath, Context.MODE_WORLD_READABLE);
				while(-1!=(len=inStream.read(bytesItem)))
				{
					if(isStop)
					{
						downState = TaskType.DOWNLOADSTOP_USER;
						Task downStop = new Task(activity,TaskType.DOWNLOADSTOP_USER,null);
						BackgroundService.addTask(downStop);//响应上层应用 下载停止
						break;
					}
					curLen = curLen+len;
					outStream.write(bytesItem,0,len);
					Task downProgress = new Task(activity,TaskType.DOWN_PROGRESS,curLen);
				...
				//	isFinished = true;
					...
					Task downFinished = new Task(activity,TaskType.DOWNLOAD_FINISHED,curLen);
		...
		finally
		{
			try 
			{
				...
				if(null!=outStream)outStream.flush();
			...
			catch (Exception e2) 
			{
				...
				Task downState = new Task(activity,TaskType.RECYCLE_RESOURCE_ERROR,null);
