...
public class WebService
{
	...
	/**
	 * @param wbParams 它必须包含WS请求参数 NameSpace,Method,WSDL,SaopEdition的值也可以添加其他的请求参数 和前者一样 参数传递都是通过Key-Value的形式 
	 * @return
	 */
	public static SoapSerializationEnvelope getWSReqRes(Soap soap) throws Exception
	{
		Log.i(TAG, WSNAMESPACE+" : "+soap.wsNameSpa+" ,"+WSDL+" : "+soap.wsWsdl+" ,"+WSMETHOD+" : "+soap.wsMethod+" ,"+WSSOAPEDITION+" : "+soap.soapEdi);
		...
		if(SoapSerializationEnvelope.VER10!=soapEdt&&SoapSerializationEnvelope.VER11!=soapEdt&&SoapSerializationEnvelope.VER12!=soapEdt)
			throw new NullPointerException("webservice服务的soap协议版本没有指定");
		
		SoapObject saopObj = new SoapObject(nameSpa,method);
		//saopObj.
		//saopObj.addAttribute("Content-Type", "application/soap+xml; charset=utf-8");
		for(Map.Entry<String, Object> wbEntry:soap.getWsProperty().entrySet())
		{
		...
		SoapSerializationEnvelope envelop = new SoapSerializationEnvelope(Integer.valueOf(soapEdt));
		envelop.bodyOut = saopObj;
		if(isDotNet)
			envelop.dotNet = true;
		HttpTransportSE transport = new HttpTransportSE(wsdl,120*1000);
