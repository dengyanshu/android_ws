...
public class SMTStickSetting extends Activity implements View.OnClickListener{
	...
	public boolean getBoolean(String key){
		SharedPreferences p = MyApplication.getApp().getSharedPreferences("smtstick_setting", Context.MODE_PRIVATE);
	...
	public void onClick(View arg0) {
		...
		case R.id.button_ok1:
			if(edit_owner.getText().length() < 1){
				new AlertDialog.Builder(this).setTitle("使用者为空").setMessage("使用者不能为空").setNegativeButton("确定", null);
			}else{
				getEditValues(); 
				saveSetting();
				updateUI();
