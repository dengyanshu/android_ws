 ...
 * @description  批号数量调整
 */
public class QuantityAdjustActivity extends CommonActivity implements View.OnClickListener,View.OnFocusChangeListener
{
	...
	public void init()
	{
		...
		//radioGroup = (RadioGroup)this.findViewById(R.id.radioGroup_plusreduce);
	...
	public void refresh(Task task)
	{
		...
		switch(task.getTaskType())
		{
			case TaskType.QUANTITY_ADJUST_SCAN:
				...
				refData(lotSN);
				//Task getLotQtyTask = new Task(this, TaskType.GETLOTSNQTY,this.edtLotSN.getText().toString());
...
//					return;
				StringBuffer sbf = new StringBuffer(this.edtSysDetail.getText().toString());
	...
	public void onClick(View v)
	{
		switch(v.getId())
		{
			case R.id.btn_ensure:
				...
				showProDia();
				String[] taskDatas = new String[]{this.edtLotSN.getText().toString().trim(),this.edtAdjustNum.getText().toString().trim()};
			...
			case R.id.btn_refreshData:
				...
				{
					Toast.makeText(this, R.string.adjustLot_notNull, Toast.LENGTH_SHORT).show();
					return;
				}
				refData(lotSN);
				break;
	...
	private void changeQuantity(View v)
	{
		...
		if(R.id.labPlus==v.getId())	
			...
			qty-=1f;
		if(0>=qty)
			Toast.makeText(this, R.string.MetrialQtyNoLowZero, Toast.LENGTH_SHORT).show();
		else
	...
	private boolean subQuantityAdjust()
	{
		...
		if(!isPass)
		{
			Toast.makeText(this, R.string.adjustNum_notNull, Toast.LENGTH_SHORT).show();
			return isPass;
		}
		if(!StringUtils.isNumberal(edtAdjustNum.getText().toString().trim()))
		{
			isPass = false;
			Toast.makeText(this, getString(R.string.adjust_amount)+getString(R.string.inputData_notLegal), Toast.LENGTH_SHORT).show();
			return isPass;
		}
		double adjustNum = Double.valueOf(edtAdjustNum.getText().toString().trim());  
		if(1>adjustNum)
		{
			isPass = false;
			Toast.makeText(this, R.string.MetrialQtyNoLowZero, Toast.LENGTH_SHORT).show();
	...
	public void onFocusChange(View v, boolean hasFocus)
	{
		switch(v.getId())
		{
			case R.id.edt_batnum:
				...
				if(edtBatNum.isFocused()&&StringUtils.isEmpty(edtBatNum.getText().toString()))
				{
					edtSysDetail.setText(edtSysDetail.getText().toString()+getString(R.string.adjust_scanLotSN)+"\n\n");
					return;
				}
				break;
		}
