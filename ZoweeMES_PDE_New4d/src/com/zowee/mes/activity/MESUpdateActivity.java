 ...
 */
public class MESUpdateActivity extends Activity implements View.OnClickListener,CommonActivityInterface 
{
	
	private ProgressBar progressBar;
//	private TextView labSchedule;//用来显示当前的下载进度
	private Button btnCancel;//取消下载按钮
	//private String downPath = "http://gdown.baidu.com/data/wisegame/149235b7960ba954/changxiangpingjiaqi.apk";
	//private String storePath = "/data/data/files/1.apk";
	private DownLoader downLoader;
	private Button btnUpdate;
//	private static final String updateSource = "http://192.168.0.27:8080/MyFirst/first.action";
	...
	//	downLoader.startDown();//
		...
		existNewEdi = reqIntent.getBooleanExtra(MESUpdateModel.EXISTNEWEDI, true);
		mesUpdate = (MESUpdate)reqIntent.getSerializableExtra(MESUpdateModel.MESUPDATE);
	}
...
//    				Toast.makeText(UpdateActivity.this, R.string.download_finish, 1500).show();
...
//    				Toast.makeText(UpdateActivity.this, R.string.update_cancel, 1500).show();//由于采用的是非断点  删除下载的临时文件
//    				
//    				break;
//    		}
//    		
...
//    };
    ...
    public void onClick(View v) 
    {
    	...
    	{
    	    case R.id.btnCancel:
    	    	if(null!=downLoader)downLoader.setStop(true);//停止下载
    	    	this.finish();
    	    	break;
    	    case R.id.btnUpdate:
    	    	...
    	    	filePath = "/data/data/"+this.getPackageName()+"/files/"+mesUpdate.getAppName();  
    	    	downLoader = new DownLoader(mesUpdate.getAppUpdateURL(), mesUpdate.getAppName(), this);
    	    	downLoader.startDown();
    	    	break;
    	}
...
//	   }
	 ...
	 //  file.setReadable(true);
	   ...
	   installApk.setDataAndType(Uri.parse("file://"+filePath), "application/vnd.android.package-archive");
   ...
   public void refresh(Task task)
   {
   		switch(task.getTaskType())
   		{
			...
			case TaskType.DOWN_FAIL://提示下载失败
				Toast.makeText(MESUpdateActivity.this, R.string.download_fail, 1500).show();     
				delDownFiles();
				break;  
			...
			case TaskType.DOWNLOAD_FINISHED:
				Toast.makeText(MESUpdateActivity.this, R.string.download_finish, 1500).show();
				progressBar.setProgress(Integer.valueOf(task.getTaskData().toString()));
				///labSchedule.setText(getString(R.string.schedule)+" "+100+"%");
				installApk();
			//	delDownFiles();
				break;
			case TaskType.DOWNLOADSTOP_USER:
				Toast.makeText(MESUpdateActivity.this, R.string.update_cancel, 1500).show();//由于采用的是非断点  删除下载的临时文件
				delDownFiles();
				break;
   		}
