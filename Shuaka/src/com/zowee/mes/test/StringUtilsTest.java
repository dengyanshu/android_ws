...
public class StringUtilsTest extends AndroidTestCase {
	...
	public void testMatStrIndex() throws Exception {
		...
		// if(isHasData)
		// String str =
		// "SQLDataSet=anyType{Return_Value=-1; I_ReturnMessage=当前批号不存在,请检查!; }; SQLDataSet1=anyType{Column1=        -1; Column2=当前批号不存在,请检查!; }; ";
		...
		// List<int[]> useDataMatches = StringUtils.getMatStrIndex(useDateRegex,
		// sign_1);
		// Log.i(TAG, "有效数据个数为: "+useDataMatches.size());
		// if(null !=matches)
		// {
		// Log.i(TAG, "匹配的个数为：" +matches.size()+"\n");
		// Log.i(TAG, sign_1.substring(0,
		// matches.get(matches.size()-2)[0]));String sign_4 =
		// "SQLDataSet=anyType\\{";
		...
		// Log.i(TAG,
		// "第一此裁剪："+sign_1.substring(matches.get(0)[1],matches.get(matches.size()-1)[0]-1));
		// Log.i(TAG, "下标值:"+matches.get(matches.size()-2)[0]);
		// Log.i(TAG,
		// "第二次裁剪: "+sign_1.substring(matches.get(1)[1],matches.get(matches.size()-2)[0]-1));
		// sign_1 =
		// sign_1.substring(matches.get(1)[1],matches.get(matches.size()-2)[0]-1);
		...
		// }
		// String str =
		// "GetDataSetWithSQLStringResponse{GetDataSetWithSQLStringResult=anyType{schema=anyType{element=anyType{complexType=anyType{choice=anyType{element=anyType{complexType=anyType{sequence=anyType{element=anyType{}; element=anyType{}; element=anyType{}; }; }; }; }; }; }; };"
		...
		// List<String> wsDataSet =
		// MesWebService.WsDataParser.getResDatSet(str);
		// List<HashMap<String,String>> resMapsLis =
		// MesWebService.getResMapsLis(resDataSet);
		// //
		// //
		// //
		...
		// String[] setting =
		// this.getContext().getResources().getStringArray(R.array.settings);
		// Log.i(TAG,setting[0]);
