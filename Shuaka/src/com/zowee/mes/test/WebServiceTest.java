...
public class WebServiceTest extends AndroidTestCase {
	...
	public void testGetWsRepRes() throws Exception {
		...
		// wsParas.put(WebService.WSDL,
		// "http://webservice.webxml.com.cn/WebServices/MobileCodeWS.asmx");
		// wsParas.put(WebService.WSSOAPEDITION,
		...
		// this.getClass().getClassLoader().getResourceAsStream("wsconfig.properties");
		...
		// }
		// Soap soap = new
		// Soap("http://tempuri.org/","GetDataSetWithSQLString","http://mes.zowee.com.cn:8008/MobileService.asmx",SoapSerializationEnvelope.VER11);
		// soap.setDotNet(true);
		//
		// soap.addWsProperty("SQLString",
		// "Create proc fifoScan @lotSn nvarchar(30) = '' As Begin if SUBSTRING(@lotSn,1,1) <>'M' begin "+
		// //
		...
		// // soap.addWsProperty("SQLString",
		// "insert into POItem(POId,ProductId,ItemQtyRequired,ItemQtyReceived,ItemQtyRejected) values('D93BC38A-EB0','PRD100006E6K',20,20,0)");
		// soap.addWsProperty("SQLString",
		...
		soap.addWsProperty(
				"SQLString",
				"declare @retVal int declare @outDetail nvarchar(max) declare @excep nvarchar(100)  exec @retVal = Txn_MaterialsRewind '',@outDetail output,@excep output, "
						+ " '','','"
		...
		//
		// soap.addWsProperty("SQLString", "select l.lotSN from lot l "
		// +" inner join LotNumber ln on l.LotId = ln.LotId" );
		...
		List<String> dataSet = MesWebService.WsDataParser
				.getResDatSet(envelop.bodyIn.toString());
		List<HashMap<String, String>> lisMaps = MesWebService
				.getResMapsLis(dataSet);
		String ONE = "SLotNO";
		...
		for (int i = 0; i < lisMaps.size(); i++) {
			...
			Log.i(TAG, mapItem.get(ONE) + "  " + mapItem.get(TWO) + " "
					+ mapItem.get(THREE) + " " + mapItem.get(FOUR));
		}

		// Log.i(TAG, envelop.bodyIn+"");
		// Editor editor =
		// this.getContext().getSharedPreferences("log",this.getContext().MODE_PRIVATE).edit();
	...
	public void testByteToStr() throws Exception {
		...
		// Log.i(TAG, new String(bytes,"UTF-8")+""+new
		...
		Log.i(TAG,
				HexStrConver.isContainByte(bytes,
						LaserScanOperator.ERROR_BARCODEBYTE) + "");
	}
	...
	public void testGetEmptySoap() throws Exception {
		...
		Log.i(TAG,
				"" + mesUpdate.getAppName() + " " + mesUpdate.getAppUpdateURL()
						+ " " + mesUpdate.getAppVersion());
