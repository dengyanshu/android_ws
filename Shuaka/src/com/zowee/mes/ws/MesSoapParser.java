...
public class MesSoapParser {
	...
	public static Soap getSoap() {
		...
		// MesSoapParser.class.getClassLoader().getResourceAsStream(WSCONFIG_INPUTPATH);
	...
	public static void mergerSoap(Soap soap) {
		...
		SharedPreferences preferences = MyApplication.getApp()
				.getSharedPreferences(WSCONFIGFILENAME, Context.MODE_PRIVATE);
		...
		if (null != soap.getWsProperty().get(USERTICKET))
			editor.putString(USERTICKET, soap.getWsProperty().get(USERTICKET)
					.toString());
	...
	public static Soap getOfficalSoap() {
		...
		if (StringUtils.isEmpty(wsNameSpace) && StringUtils.isEmpty(wsWSDL))// 用户从原始文件中读取激光配置信息
		{
			Soap soap = getSoap();
			mergerSoap(soap);
		}
		...
		String userTicket = preferences.getString("pref_user_ticket", ""); // change
																			// by
																			// ybj
