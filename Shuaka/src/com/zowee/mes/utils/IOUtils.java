...
public class IOUtils {
	...
	public static byte[] parseInStreamToBytes(InputStream inStream) {
		if (null == inStream)
			throw new NullPointerException("inStream can not be null");
	 ...
	 */
	public static String parseInStreamToStr(InputStream inStream,
			String encoding) {
		...
		// Log.i(TAG, "111");
		if (null == inStreamBytes || 0 == inStreamBytes.length)
			return null;
		String strVal = null;
	 ...
	 */
	public static String parBloInStreamToStr(InputStream inStream,
			String encoding) {
		if (StringUtils.isEmpty(encoding))
			encoding = "UTF-8";
		byte[] inStreamBytes = parBloInStreamToBytes(inStream);
		if (null == inStreamBytes)
			return null;
	 ...
	 */
	public static boolean parseInstreamToOutstream(InputStream instream,
			OutputStream out) {
		try {
