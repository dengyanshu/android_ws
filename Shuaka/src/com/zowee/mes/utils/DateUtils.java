...
public class DateUtils {
	 ...
	 * @param regex
	 * @return
	 * @description 把日期转换成指定的字符形式 例如 regex yyyy-MM-dd hh-mm-ss = 2012-09-14
	 *              12:30:49
	 */
	...
	public static String repairDate(String datetime) {
		...
		if (2 == last - first && 1 == datetime.length() - last) {
			datetime = datetime.substring(0, first) + "0"
					+ datetime.substring(first);
			datetime = datetime.substring(0, last + 1) + "0"
					+ datetime.substring(last + 1);
		} else if (2 == last - first)
			datetime = datetime.substring(0, first) + "0"
					+ datetime.substring(first);
		else if (1 == datetime.length() - last)
			datetime = datetime.substring(0, last) + "0"
					+ datetime.substring(last);
