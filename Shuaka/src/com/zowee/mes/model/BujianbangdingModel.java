...
public class BujianbangdingModel extends CommonModel {
	
	public void bujianbangding(Task task) {
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					String sn = Params[4];
					String lcdsn = Params[5];
					String  tp=Params[6];
					String dianchisn = Params[7];
					String shexiangtousn = Params[8];

					Log.i(TAG, "任务数据是:" + Params[0] + "," + Params[1] + ","
							+ Params[2] + "," + Params[3] + "," + Params[4]
							+ "," + Params[5]+Params[6]);

					String sql = " declare @res int declare @message nvarchar(max) declare @exception nvarchar(100)  exec @res= [txn_lenovo_PAD_ASS]    "
							+" @I_OrBitUserId='"+useid+"',@I_OrBitUserName='"+usename+"',@I_ResourceId='"+resourceid+"',@I_ResourceName='"+resourcename+"',"
							+ "@LCD='"+lcdsn+"',@TP='"+tp+"',@Aftercamera='"+shexiangtousn+"',@PowerSN='"+dianchisn+"',@PCBASN='"+sn+"',"
