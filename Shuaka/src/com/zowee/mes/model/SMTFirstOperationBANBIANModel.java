...
public class SMTFirstOperationBANBIANModel extends CommonModel {
		...
		TaskOperator taskOperator = new TaskOperator() {
			...
			public Task doTask(Task task) {
				try {
					...
					kouliaoNum = taskdata.get("kouliaoNum");
					...
					if (customer.equalsIgnoreCase("oppo")) // for OPPO
						...
						Sql = "declare @ReturnMessage nvarchar(max) declare @res int declare @ExceptionFieldName nvarchar(100) exec @res = Txn_SprayingScanBANBIAN_Batch @I_ReturnMessage=@ReturnMessage out,@I_ExceptionFieldName=@ExceptionFieldName  out,@I_OrBitUserId='"
								+ MyApplication.getMseUser().getUserId()
								+ "',@I_OrBitUserName='"
								+ MyApplication.getMseUser().getUserName()
								+ "',@I_ResourceName='"
