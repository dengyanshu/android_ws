 ...
 * @description 物料盘点
 */
public class MaterialInventoryActivity extends CommonActivity implements
	...
	public void init() {
		...
		edtStockLocation = (EditText) this
				.findViewById(R.id.edt_storage_locaction);
		...
		edtSysDetails = (EditText) this.findViewById(
	...
	public void refresh(Task task) {
		...
		case TaskType.METERIAL_INVENTORY_SCAN:
			...
			if (0 == focusMark) {
				...
				// Task GET_PARAMS = new Task(this, TaskType.MET_INV_GETPARAMS,
				// lotSN);
				// model.getNecessaryParams(GET_PARAMS);//开启一个任务 根据物料批号 获取 库位
				// ,物料数量
				refData(lotSN);
			} else if (1 == focusMark)
		...
		case TaskType.METERIAL_INVENTORY_CK:
			...
			StringBuffer sbf = new StringBuffer(edtSysDetails.getText()
					.toString());
			if (!StringUtils.isEmpty(retRes) && "0".equals(retRes.trim())) {
				...
				// showProDia();
				Task SMTask = new Task(this, TaskType.METERIAL_INVENTORY_SM,
						pars);// 数据保存
		...
		case TaskType.METERIAL_INVENTORY_SM:
			...
			StringBuffer sbf1 = new StringBuffer(edtSysDetails.getText()
					.toString());
			if (!StringUtils.isEmpty(retResSave)
					&& "0".equals(retResSave.trim()))
	...
	public void onClick(View v) {
		...
		case R.id.btn_ensure:
			...
			String[] params = new String[] {
					edtLotSN.getText().toString().trim(),
					this.edtStockLocation.getText().toString().trim(),
					this.edtQTY.getText().toString().trim(),
					edtRepeatLotSN.getText().toString().trim(), CK };
		...
		case R.id.btn_refreshData:
			...
			if (StringUtils.isEmpty(lotSN)) {
				Toast.makeText(this, R.string.materialNum_notNull,
	...
	private boolean subWidget() {
		...
		if (!isPass) {
			Toast.makeText(
					...
					getString(R.string.stockLocation) + " "
							+ getString(R.string.inputData_notNull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		...
		if (!isPass) {
			Toast.makeText(
					this,
					getString(R.string.meterial_amount) + " "
							+ getString(R.string.inputData_notNull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		...
		if (!isPass) {
			Toast.makeText(
					this,
					getString(R.string.repeatScan) + " "
							+ getString(R.string.inputData_notNull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		if (!edtLotSN.getText().toString().trim()
				.equals(edtRepeatLotSN.getText().toString().trim())) {
			isPass = false;
			Toast.makeText(this, R.string.matInv_dataDisaccord,
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
	...
	public void onFocusChange(View v, boolean hasFocus) {
		...
		case R.id.edt_material_batnum:
			...
			if (edtMatBatNum.isFocused()
					&& StringUtils.isEmpty(edtMatBatNum.getText().toString()))
				edtSysDetails.setText(edtSysDetails.getText()
						+ getString(R.string.please_scan) + " "
						+ getString(R.string.meterial_batnum) + "\n\n");
			break;
		case R.id.edt_repeat_batnum:
			EditText edtRepBatNum = (EditText) v;
			if (edtRepBatNum.isFocused()
					&& StringUtils.isEmpty(edtRepBatNum.getText().toString()))
				edtSysDetails.setText(edtSysDetails.getText()
						+ getString(R.string.please_scan) + " "
						+ getString(R.string.repeatScan)
						+ getString(R.string.meterial_batnum) + "\n\n");
		...
		case R.id.edt_storage_locaction:
			...
			if (edtStorageLoc.isFocused()
					&& StringUtils.isEmpty(edtStorageLoc.getText().toString()))
				edtSysDetails.setText(edtSysDetails.getText()
						+ getString(R.string.scan_storage_position) + "\n\n");
			break;
		}
