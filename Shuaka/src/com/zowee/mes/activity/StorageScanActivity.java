...
public class StorageScanActivity extends CommonActivity implements
	...
	public void init() {
		...
		Params = new String[] { "", "", "", "" };
		edtOutPackageNum = (EditText) this
				.findViewById(R.id.edt_outbox_packNum);
		edtGRNNO = (EditText) this.findViewById(R.id.edt_reciegood_num);
		SpiLocaPosition = (Spinner) this
				.findViewById(R.id.Spinner_storage_locaction);
	...
	public void refresh(Task task) {
		...
		case TaskType.STORAGE_SCAN_NETWORK:
			...
			if (Integer.parseInt(getdata.get("result").toString()) == 0) {
				logDetails(edtSysDetails,
						"Success_Msg:" + getdata.get("ReturnMsg").toString()
								+ "\r\n");
	...
	public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
		...
		case R.id.Spinner_storage_locaction:
			if (null == StockCodes || 0 == StockCodes.size())
				return;
	...
	private void GetResourceId() {
		...
		logDetails(edtSysDetails, "资源名称："
				+ MyApplication.getAppOwner().toString());
		Task task = new Task(this, TaskType.GetResourceId, MyApplication
				.getAppOwner().toString());
	...
	private boolean saveDataToService() {
		...
		// ybj String[] paras =
		// {this.edtOutPackageNum.getText().toString().trim(),this.edtLocaPosition.getText().toString().trim(),this.edtGRNNO.getText().toString().trim()};
	...
	private boolean subWidget() {
		...
		if (!isPass_1) {
			Toast.makeText(this, R.string.GRNNO_NOTNULL, Toast.LENGTH_SHORT)
					.show();
			return isPass_1;
		}
		if (!isPass_2) {
		...
		if (!isPass_3) {
			Toast.makeText(this, R.string.SmallBall_NOTNULL, Toast.LENGTH_SHORT)
					.show();
			return isPass_3;
		}
	 ...
	 * 
	 * @param lotsn
	 *            将扫描到的数据应用到UI界面 return true = 表明该扫描是对外箱号或小包号进行的 需要和服务器同步数据
	 */
	private boolean applyScanData(String lotsn) {
		...
		return false;
		/*
		 * if(StringUtils.isScannedStorageSlot(lotsn)) // changed by ybj
		 * 2014-02-18 { edtLocaPosition.setText(lotsn);
		 * edtOutPackageNum.setText(""); edtOutPackageNum.requestFocus(); } else
		 * { if(edtGRNNO.isFocused()){ edtGRNNO.setText(lotsn);
		 * edtLocaPosition.requestFocus(); }else
		 * if(edtLocaPosition.isFocused()){ edtLocaPosition.setText(lotsn);
		 * edtOutPackageNum.requestFocus(); }else
		 * if(edtOutPackageNum.isFocused()) { edtOutPackageNum.setText(lotsn);
		 * return true; }else{ edtLocaPosition.setText(lotsn);
		 * edtOutPackageNum.requestFocus(); } } return false;
		 */
	}
