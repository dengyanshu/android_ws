...
public class SplitStoreActivity extends CommonActivity implements
	...
	public void init() {
		...
		edtSysDatail = (EditText) this.findViewById(
	...
	public void refresh(Task task) {
		...
		case TaskType.SPLIT_STORAGE_WS:
			...
			if (StringUtils.isEmpty(logo))
				logo = "";
			sbf.append(logo + "" + "\n\n");
			this.edtSysDatail.setText(this.edtSysDatail.getText().toString()
					+ sbf.toString());
			break;
	...
	private void showScanData(Task task) {
		...
		if (edtSplitLot.isFocused()) {
			...
			// TaskType.STORAGE_SPLITE_GETPARAMS, lotSN);
	...
	public void onClick(View v) {
		...
		case R.id.btn_ensure:
			...
			if (!isPass)
				return;//
			Object[] taskData = { edtSplitLot.getText().toString().trim(),
					Double.valueOf(edtSplitAmount.getText().toString().trim()),
					edtTargetLot.getText().toString().trim() };
		...
		case R.id.btn_refreshData:
			...
			if (StringUtils.isEmpty(lotSN)) {
				Toast.makeText(this, R.string.splitlot_notnull,
	...
	private boolean subStoreSplitData() {
		...
		if (!isPass) {
			Toast.makeText(this, getString(R.string.splitlot_notnull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		...
		if (!isPass) {
			Toast.makeText(this, getString(R.string.splitamount_notnull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		if (!StringUtils.isNumberal(edtSplitAmount.getText().toString().trim())) {
			...
			Toast.makeText(
					this,
					getString(R.string.slit_amount)
							+ getString(R.string.inputData_notLegal),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}

		if (0 >= Double.valueOf(edtSplitAmount.getText().toString().trim())
				.doubleValue()) {
			isPass = false;
			Toast.makeText(this, getString(R.string.splitAmount_notLowZero),
					1500).show();
		...
		if (!isPass) {
			Toast.makeText(this, getString(R.string.targetlot_notnull),
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
	...
	public void onFocusChange(View v, boolean hasFocus) {
		...
		case R.id.edt_split_batnum:
			if (tempEdt.isFocused()
					&& StringUtils.isEmpty(tempEdt.getText().toString()))
				edtSysDatail.setText(edtSysDatail.getText()
						+ getString(R.string.scan_splitLot) + "\n\n");
			break;
		case R.id.edt_target_batnum:
			if (tempEdt.isFocused()
					&& StringUtils.isEmpty(tempEdt.getText().toString()))
				edtSysDatail.setText(edtSysDatail.getText()
						+ getString(R.string.scan_targetLot) + "\n\n");
			break;
		}
	...
	private void refData(String lotSN) {
		super.progressDialog.setMessage(getString(R.string.loadAppNecessData));
		showProDia();
		Task getParams = new Task(this, TaskType.STORAGE_SPLITE_GETPARAMS,
				lotSN);
