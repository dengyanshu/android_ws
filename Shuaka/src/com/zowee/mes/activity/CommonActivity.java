...
public abstract class CommonActivity extends Activity implements
	...
	public void refresh(Task task) {
		...
		case TaskType.WEBSERVICE_FAIL:
			Toast.makeText(this, R.string.ws_req_fail, Toast.LENGTH_LONG)
					.show();
			break;
		case TaskType.NET_CONNECT_FAIL:
			Toast.makeText(this, R.string.NetConFail, Toast.LENGTH_SHORT)
					.show();
			break;
		}
	}
	 ...
	 * 
	 * switch(keyCode) { case MyApplication.SCAN_KEYCODE:
	 * 
	 * laserScan(); break; }
	 * 
	...
	protected void laserScan() {
		if (!laserScanOperator.getLaserEnable())
			return;
	...
	protected void clearWidget(View view, Object specialVal) {
		...
		if (view instanceof Spinner) {
			...
			if (null == specialVal)
				spinner.setAdapter(new CommonSpinnerAdapter<String>(
						new ArrayList<String>(0), this));
			else {
				SpinnerAdapter spinnerAdapter = (SpinnerAdapter) specialVal;
				spinner.setAdapter(spinnerAdapter);
			}
			return;
			// spinner.setAdapter(new ArrayAdapter<T>(this,
			// textViewResourceId));
		}
