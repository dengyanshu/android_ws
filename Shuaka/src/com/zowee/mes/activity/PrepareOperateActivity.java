 ...
 * @description 备料操作
 */
public class PrepareOperateActivity extends CommonActivity implements
	...
	public void onClick(View v) {
		...
		case R.id.edt_material_barcode:
			if (applyScanData(edtMaterialSN.getText().toString().trim())) { // Modify
																			// by
																			// ybj
																			// 20140213
				PrepareOperation();
	...
	private boolean PrepareOperation() {
		...
		if (!isPass)
			return false;
		String[] paras = { this.edtMaterialSN.getText().toString().trim(),
				this.edtLocaPosition.getText().toString().trim(),
				this.edtPrepMaterialMO.getText().toString().trim(),
				edtPrepMaterialOrderNum.getText().toString().trim() };
	...
	private boolean subWidget() {
		...
		if (!isPass_1) {
			Toast.makeText(this, R.string.moName_notNull, Toast.LENGTH_SHORT)
					.show();
			return isPass_1;
		}
		if (!isPass_2) {
		...
		if (!isPass_3) {
			Toast.makeText(this, R.string.StocPosition_NOTNULL,
					Toast.LENGTH_SHORT).show();
			return isPass_3;
		}
		if (!isPass_4) {
			Toast.makeText(this, R.string.lotSN_notNull, Toast.LENGTH_SHORT)
					.show();
			return isPass_4;
		}
		return true;
	...
	public void onFocusChange(View v, boolean hasFocus) {
		...
		case R.id.edt_storage_locaction:
			if (edtLocaPosition.isFocused()) {
				edtPrepMaterialMO.setText(edtPrepMaterialMO.getText()
						.toString().toUpperCase());
				edtPrepMaterialOrderNum.setText(edtPrepMaterialOrderNum
