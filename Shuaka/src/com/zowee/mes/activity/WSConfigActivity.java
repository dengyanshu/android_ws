 ...
 */
public class WSConfigActivity extends CommonActivity implements
	...
	public void init() {
		...
		// edtSqlSentenceName =
		// (EditText)this.findViewById(R.id.edt_sqlSentence);
	...
	private void initUIData(Soap soap) {
		...
		if (null != soap.getWsProperty().get(USERTICKET))
			edtUserTicket.setText(soap.getWsProperty().get(USERTICKET)
					.toString());
		if (soap.isDotNet()) {
	...
	private boolean subUIData() {
		...
		if (!isPass) {
			Toast.makeText(this, R.string.wsSpaceName_isnull,
					Toast.LENGTH_SHORT).show();
			return isPass;
		}
		isPass = subView(edtWSDL);
		if (!isPass) {
			Toast.makeText(this, R.string.wsWsdl_isnull, Toast.LENGTH_SHORT)
					.show();
			return isPass;
		}
		isPass = subView(edtWSEdition);
		if (!isPass) {
			Toast.makeText(this, R.string.wsEdition_isnull, Toast.LENGTH_SHORT)
					.show();
			return isPass;
		}
		isPass = subView(edtWSMethod);
		if (!isPass) {
			Toast.makeText(this, R.string.wsMethod_isnull, Toast.LENGTH_SHORT)
					.show();
			return isPass;
		}
