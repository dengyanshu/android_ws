...
public class SerialPortOper {
	...
	private final static int[] DATA_BITS = { SerialPort.DATABITS_5,
			SerialPort.DATABITS_6, SerialPort.DATABITS_7, SerialPort.DATABITS_8 };
	private final static int[] FLOW_CONTROL = { SerialPort.FLOWCONTROL_NONE,
			SerialPort.FLOWCONTROL_RTSCTS_IN,
			SerialPort.FLOWCONTROL_RTSCTS_OUT,
			SerialPort.FLOWCONTROL_XONXOFF_IN,
			SerialPort.FLOWCONTROL_XONXOFF_OUT };
	private final static int[] PARITYS = { SerialPort.PARITY_EVEN,
			SerialPort.PARITY_MARK, SerialPort.PARITY_NONE,
			SerialPort.PARITY_ODD, SerialPort.PARITY_SPACE };
	...
	private void init(SerialParams serialParams) throws Exception {
		...
		CommPortIdentifier portId = CommPortIdentifier
				.getPortIdentifier(serialParams.name);
		serialPort = (SerialPort) portId.open(serialParams.owner,
				serialParams.timeOut);
		inStream = serialPort.getInputStream();
		outStream = serialPort.getOutputStream();
		serialPort.setSerialPortParams(serialParams.baudRate,
	...
	public final static class SerialParams implements Serializable {
		...
		public SerialParams(String name, String owner, int timeOut,
				int baudRate, int dataBits, int endBits, int parity,
				int flowControl) {
			super();
			this.name = name;
			this.owner = owner;
	...
	private boolean subSerialParam() {
		if (null == serialParams)
			return false;
		if (StringUtils.isEmpty(serialParams.name))
			return false;
		if (StringUtils.isEmpty(serialParams.owner))
			return false;
		if (serialParams.timeOut <= -1)
			return false;
		if (serialParams.baudRate <= 0)
			return false;
		if (!ArrayUtils.findValInIntArr(serialParams.dataBits, DATA_BITS))
			return false;
		if (!ArrayUtils.findValInIntArr(serialParams.endBits, STOP_BITS))
			return false;
		if (!ArrayUtils.findValInIntArr(serialParams.parity, PARITYS))
			return false;
		if (!ArrayUtils.findValInIntArr(serialParams.flowControl, FLOW_CONTROL))
			return false;
